...
const devicesId = "561953574" // 填写在OneNet上获得的devicesId 形式就是一串数字 例子:9939133
const api_key = "jbS6UIjyvXQW2fkez2GpPBp15eE=" // 填写在OneNet上的 api-key 例子: VeFI0HZ44Qn5dZO14AuLbWSlSlI=
...
Page({
  ...
  getDatapoints: function () {
    return new Promise((resolve, reject) => {
      wx.request({
        url: `https://api.heclouds.com/devices/${devicesId}/datapoints?datastream_id=T,L,H&limit=20`,
        /**
         * 添加HTTP报文的请求头, 
        ...
        success: (res) => {
          ...
          resolve({
            T: response.data.datastreams[0].datapoints.reverse(),
            // light: response.data.datastreams[1].datapoints.reverse(),
            H: response.data.datastreams[1].datapoints.reverse()
          })
        },
        fail: (err) => {
          reject(err)
  ...
  convert: function (datapoints) {
    ...
    var length = datapoints.H.length
    for (var i = 0; i < length; i++) {
      categories.push(datapoints.H[i].at.slice(5, 19));
      H.push(datapoints.H[i].value);
      // light.push(datapoints.light[i].value);
      T.push(datapoints.T[i].value);
    }
    return {
